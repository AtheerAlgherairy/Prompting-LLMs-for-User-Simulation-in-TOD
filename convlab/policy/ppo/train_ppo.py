...
def sampler(env, policy, num_dialogues, train_seed=0, user_reward=False):
    """
    This is a sampler function, and it will be called by sample function to sample data from environment
    ...
    while sampled_traj_num < num_dialogues:
        ...
        for t in range(traj_len):
            ...
            # print(f"a     : {a}")
            # interact with env
            next_s, r, done = env.step(a, user_reward=False)
...
def sample(env, policy, num_train_dialogues, process_num, seed, user_reward=False):
    ...
    for i in range(process_num):
      buff.append(sampler(env, policy, process_num_dialogues, train_seeds[i], user_reward))
...
def update(env, policy, num_dialogues, epoch, process_num=1, seed=0, user_reward=False):
...
if __name__ == '__main__':
    ...
    for i in range(conf['model']['epoch']):
        ...
        update(env, policy_sys, conf['model']['num_train_dialogues'], idx, conf['model']['process_num'], seed=seed, user_reward=False)
